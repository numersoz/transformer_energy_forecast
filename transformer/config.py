...
class Config:
    def __init__(self, data='ELECT1',seq_len=720, pred_len=24, dec_seq_len=24, hidden_size=128, heads=3, batch_size=100, embedding_size=32,
                 n_encoder_layers=3, n_decoder_layers=3,lr = 0.001, lradj = '',
                 prediction_type='uni', dropout=0.1, fp16=True, 
                 iterations=10, exps=5, debug=False):
        self.data = data
    ...
    def extend_argv(self):
        ...
        sys.argv.extend(["--lr", str(self.lr)])        
        sys.argv.extend(["--lradj", str(self.lradj)])        
    ...
    def __str__(self):
        ...
        res += ':::: model learning rate: {}\n'.format(self.lr)
        res += ':::: model learning rate adjacent: {}\n'.format(self.lradj)
...
def build_parser():
    ...
    parser.add_argument('--data', type=str, choices=['ELECT1', 'ELECT2'], required=True)
    ...
    parser.add_argument('--lradj', type=str, required=False)
    parser.add_argument('--target', default='electricity', type=str)
